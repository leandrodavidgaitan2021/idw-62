...
function rellenarModal(medico) {
  ...
  const valorConsultaStr =
    medico.valorConsulta > 0
  ...
  document.getElementById(
    "detalleMedicoNombreCompleto"
  ).textContent = `${medico.nombre} ${medico.apellido}`;
  document.getElementById("detalleMedicoEspecialidad").textContent =
    medico.getEspecialidadNombre();
  document.getElementById("detalleMedicoMatricula").textContent =
    medico.matricula || "N/A";
  document.getElementById("detalleMedicoValorConsulta").textContent =
    valorConsultaStr;
  document.getElementById("detalleMedicoDescripcion").textContent =
    medico.descripcion ||
    "No hay descripción disponible para este profesional.";
  ...
  if (obrasSocialesNombres && obrasSocialesNombres.length > 0) {
    ...
    ulObrasSociales.innerHTML =
      '<li><i class="fa-solid fa-triangle-exclamation text-warning me-2"></i> Solo atiende consultas privadas.</li>';
  }
  ...
  btnNuevaReservaModal.replaceWith(btnNuevaReservaModal.cloneNode(true));
...
function calcularPrecioFinal() {
  const selectorOS = document.getElementById("selectorObraSocial");
  const displayPrecioFinal = document.getElementById(
    "detalleMedicoPrecioFinal"
  );
  ...
  if (selectorOS.value === "") {
    displayPrecioFinal.textContent = precioBase.toLocaleString("es-AR", {
      style: "currency",
      currency: "ARS",
    });
  ...
  displayPrecioFinal.textContent = precioFinal.toLocaleString("es-AR", {
    style: "currency",
    currency: "ARS",
  });
...
function renderizarMedicos(lista) {
  ...
  lista.forEach((medico) => {
    ...
    const btnInfo = document.createElement("a"); // mejor usar <a> si tiene href
    btnInfo.className = "btn btn-info btn-sm fw-semibold shadow-sm";

    // Generar la URL
    const urlDetalle = medico.generarUrlDetalle
      ? medico.generarUrlDetalle()
      : `medico-detalle.html?id=${medico.id}`;
    ...
    body.append(h5, especialidad, obras, btnInfo);
